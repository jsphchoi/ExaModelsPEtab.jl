...
#   Zheng (singular-KKT/unidentifiable, lg(rg) pinned ~9–10); Bachmann (compiles at K=4 but
#   solver-fails — NOT an OOM); Borghans/Elowitz (NaN-spin); event models (don't build/diverge).
...
const EXA_RERUN_INLOOP = [   # benchmark_examodels.jl timed loop, clean → unreliable order
    ...
    "SalazarCavazos_MBoC2020", "Crauste_CellSystems2017",
...
const EXA_RERUN_MODELS = [EXA_RERUN_INLOOP; "Bruno_JExpBot2016"]  # 15 (in-loop 14 + Bruno)

@assert length(EXA_RERUN_INLOOP) == 14
